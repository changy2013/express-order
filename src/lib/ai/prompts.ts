...
## 各 specialMode 用法
- **normal**：标准行列表格。每行一个SKU，表头在headerRow，footerConfig可从底部提取收货人。若文件首行有标题/标签行（如行列标题上方的"仓库名称 货主名称…"），用 dataStartRow 跳过它们，把 headerRow 指到真正的字段表头行。

- **matrix**：SKU在行、门店在列、交叉单元格为数量（如"银泰 | 金银潭 | 金桥"下有 1/空/3）。用 matrixConfig，引擎自动按门店列拆成多行——对每个 SKU 遍历所有门店列，有数量（>0）则生成一行，空/0 则跳过不导入。配置要点：
  - storeHeaderRow：门店名列所在行号（通常是表头行）
  - storeStartCol：第一个门店列的索引（不含左侧固定字段如 SKU编码/名称 等）
  - storeEndCol：最后一个真实门店列的索引，⚠️ 务必排除汇总列：如"下单后结余""剩余""库存""可用""冻结""分配"等计算/统计列 —— 这些列的数值不是门店订货量，视为非门店列
  - storeSkipKeywords：额外需要跳过的列名关键词（引擎已内置 ['结余','合计','小计','汇总','总和','库存','数量','可用','冻结','分配','待']，无需重复填写；如需新增其它关键词才填）
  - skuCodeCol / skuNameCol / skuSpecCol：SKU 编码/名称/规格 的列索引（不是用 fieldMappings）
  - 收货门店/收件人 若不在矩阵表中，用 staticValues 写死或用 footerConfig 从头部/尾部提取

- **card**：卡片堆叠，每卡片以cardMarker开头，卡片头部有收货人/门店，下面接物品小表。用cardConfig。⚠️卡片末尾常有「合计：N 个门店 | N 种物品 | 总数量…」这类汇总行——务必把「合计/小计/汇总/总计」等加进skipRowPatterns，否则汇总行会被误解析成一条物品。若文档顶部有统一单号（如「调拨单号：DB...」）而各卡片自身无单号，用externalCodeGen让引擎按"单号-卡片序号"为每张卡片派生唯一外部编码。
